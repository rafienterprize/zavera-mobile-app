...
DECLARE
    ...
    order_code TEXT;
    order_status TEXT;
    statuses TEXT[] := ARRAY['PAID', 'PACKING', 'SHIPPED', 'DELIVERED'];
BEGIN
    FOR i IN 1..50 LOOP
        ...
        VALUES (
            ...
            CASE WHEN order_status != 'PENDING' THEN NOW() - (random() * INTERVAL '50 days') ELSE NULL END
        )
...
SELECT 
    ...
    CASE 
        ...
        WHEN random() < 0.92 THEN 'OUT_FOR_DELIVERY'
        WHEN random() < 0.96 THEN 'PICKUP_SCHEDULED'
